...
class CcpSender {
    ...
    handleRouteControl({ mode, lastKnownRoutingTableId, lastKnownEpoch, features }) {
        if (this.mode !== mode) {
            this.log.debug('peer requested changing routing mode. oldMode=%s newMode=%s', ilp_protocol_ccp_1.ModeReverseMap[this.mode], ilp_protocol_ccp_1.ModeReverseMap[mode]);
        }
        this.mode = mode;
